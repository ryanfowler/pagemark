...
`gate.json` records the observed Pagemark baseline. Aggregate minima are rounded down to four decimal places; extraction errors and metadata exact-match counts use the observed values. Per-fixture content scores and metadata match states are retained only to identify regressions. Regenerate the gate only after reviewing a compatibility change with the corpus evaluation harness for the current release.
...
To upgrade, choose and review an upstream commit; update the gitlink, corpus constants, CI pin checks, and the commit/tree/count above; run the complete corpus evaluation; review score changes and worst fixtures; then deliberately update `gate.json` and the baseline report. Never fetch or refresh fixtures during a normal test.
